...
namespace ArtnetEmu.Model
{
    ...
    public abstract class MediaPlayer<FixtureType, Config> : IFileContainer
        ...
        where Config : PlayerConfiguration, new()
    {
        ...
        public void PacketReceived(ArtNetPacket packet)
        {
            if (packet.Physical == config.Physical && packet.Universe == config.Universe)
            {
